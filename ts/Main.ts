...
export class Main {
    ...
    static trainingProjects: Map<string, boolean> = new Map<string, boolean>();
    ...
    constructor() {
        ...
        let trainingTab: Tab = new Tab('training', 'Training', false, Main.tabHolder);
        this.trainingView = new TrainingView(trainingTab.getContainer());
        trainingTab.getLabelDiv().addEventListener('click', () => {
            this.trainingView.setSizes();
        });
        Main.tabHolder.addTab(trainingTab);
        ...
        ipcRenderer.on('view-training', () => {
            Main.tabHolder.selectTab('training');
        });
        ipcRenderer.on('request-training', () => {
            this.trainingView.loadExercises();
        });
        ipcRenderer.on('register-training-project', (event: IpcRendererEvent, arg: { projectId: string }) => {
            Main.trainingProjects.set(arg.projectId, true);
        });
        ipcRenderer.on('close-training-tab', (event: IpcRendererEvent, arg: { projectId: string }) => {
            if (Main.tabHolder.has(arg.projectId) && Main.tabHolder.canClose(arg.projectId)) {
                Main.tabHolder.closeTab(arg.projectId);
                if (Main.translationViews.has(arg.projectId)) {
                    let view = Main.translationViews.get(arg.projectId) as TranslationView;
                    view.close();
                    Main.translationViews.delete(arg.projectId);
                    ipcRenderer.send('close-project', { project: arg.projectId });
                }
            }
        });
    ...
    addTab(project: Project): void {
        ...
        let isTraining: boolean = (project as any).trainingMode === true;
        let tab: Tab = new Tab(project.id, project.description, true, Main.tabHolder);
        let view: TranslationView = new TranslationView(tab, project.id, project.sourceLang, project.targetLang, Main.rowsPage, isTraining);
