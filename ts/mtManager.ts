...
export class MTManager {
    ...
    constructor(preferences: Preferences, srcLang: string, tgtLang: string) {
        ...
        if (preferences.glm.enabled) {
            let glmTranslator: GlmTranslator = new GlmTranslator(preferences.glm.apiKey, preferences.glm.endpoint, preferences.glm.model);
            glmTranslator.setSourceLanguage(srcLang);
            glmTranslator.setTargetLanguage(tgtLang);
            this.mtEngines.push(glmTranslator);
            if (preferences.glm.fixTags) {
                this.tagFixer = glmTranslator;
            }
        }
        if (preferences.doubao.enabled) {
            let doubaoTranslator: DoubaoTranslator = new DoubaoTranslator(preferences.doubao.apiKey, preferences.doubao.endpoint, preferences.doubao.model);
