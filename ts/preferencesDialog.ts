...
export class PreferencesDialog {
    ...
    enableGlm: HTMLInputElement = document.createElement('input');
    glmKey: HTMLInputElement = document.createElement('input');
    glmEndpoint: HTMLSelectElement = document.createElement('select');
    glmModel: HTMLInputElement = document.createElement('input');
    glmFixTags: HTMLInputElement = document.createElement('input');

    enableDoubao: HTMLInputElement = document.createElement('input');
    doubaoKey: HTMLInputElement = document.createElement('input');
    doubaoEndpoint: HTMLSelectElement = document.createElement('select');
    ...
    doubaoGeneralModels: string[] = [];
    doubaoCodingModels: string[] = [];
    ...
    setPreferences(preferences: Preferences): void {
        ...
        this.enableGlm.checked = preferences.glm.enabled;
        this.glmKey.value = preferences.glm.apiKey;
        this.glmEndpoint.value = preferences.glm.endpoint;
        this.glmModel.value = preferences.glm.model;
        this.glmKey.disabled = !preferences.glm.enabled;
        this.glmEndpoint.disabled = !preferences.glm.enabled;
        this.glmModel.disabled = !preferences.glm.enabled;
        this.glmFixTags.checked = preferences.glm.fixTags;
        this.glmFixTags.disabled = !preferences.glm.enabled;
        this.enableGlm.addEventListener('change', () => {
            this.glmKey.disabled = !this.enableGlm.checked;
            this.glmEndpoint.disabled = !this.enableGlm.checked;
            this.glmModel.disabled = !this.enableGlm.checked;
            this.glmFixTags.disabled = !this.enableGlm.checked;
        });

        this.enableDoubao.checked = preferences.doubao.enabled;
        this.doubaoKey.value = preferences.doubao.apiKey;
        this.doubaoEndpoint.value = preferences.doubao.endpoint;
        this.doubaoModel.value = preferences.doubao.model;
        this.doubaoKey.disabled = !preferences.doubao.enabled;
        this.doubaoEndpoint.disabled = !preferences.doubao.enabled;
        this.doubaoModel.disabled = !preferences.doubao.enabled;
        this.doubaoFixTags.checked = preferences.doubao.fixTags;
        this.doubaoFixTags.disabled = !preferences.doubao.enabled;
        this.enableDoubao.addEventListener('change', () => {
            this.doubaoKey.disabled = !this.enableDoubao.checked;
            this.doubaoEndpoint.disabled = !this.enableDoubao.checked;
    ...
    savePreferences(): void {
        ...
        if (this.enableGlm.checked && this.glmKey.value === '') {
            ipcRenderer.send('show-message', { type: 'warning', message: 'Enter GLM API key', parent: 'preferences' });
            return;
        }
        if (this.enableGlm.checked && this.glmModel.value === '') {
            ipcRenderer.send('show-message', { type: 'warning', message: 'Enter GLM model', parent: 'preferences' });
            return;
        }
        if (this.enableDoubao.checked && this.doubaoKey.value === '') {
            ipcRenderer.send('show-message', { type: 'warning', message: 'Enter Doubao API key', parent: 'preferences' });
            return;
        }
        ...
        let prefs: Preferences = {
            ...
            glm: {
                enabled: this.enableGlm.checked,
                apiKey: this.glmKey.value,
                endpoint: this.glmEndpoint.value,
                model: this.glmModel.value,
                fixTags: this.glmFixTags.checked
            },
            doubao: {
    ...
    populateMtTab(container: HTMLDivElement): void {
        ...
        let glmRadio: HTMLInputElement = document.createElement('input');
        glmRadio.type = 'radio';
        glmRadio.name = 'mtProvider';
        glmRadio.id = 'glmRadio';
        glmRadio.style.margin = '4px';
        radioRow.appendChild(glmRadio);

        let glmLabel: HTMLLabelElement = document.createElement('label');
        glmLabel.setAttribute('for', 'glmRadio');
        glmLabel.innerText = 'GLM';
        glmLabel.style.marginTop = '4px';
        glmLabel.classList.add('noWrap');
        radioRow.appendChild(glmLabel);
        ...
        let doubaoRadio: HTMLInputElement = document.createElement('input');
        doubaoRadio.type = 'radio';
        doubaoRadio.name = 'mtProvider';
        doubaoRadio.id = 'doubaoRadio';
        doubaoRadio.style.margin = '4px';
        radioRow.appendChild(doubaoRadio);

        let doubaoLabel: HTMLLabelElement = document.createElement('label');
        doubaoLabel.setAttribute('for', 'doubaoRadio');
        doubaoLabel.innerText = 'Doubao';
        doubaoLabel.style.marginTop = '4px';
        doubaoLabel.classList.add('noWrap');
        radioRow.appendChild(doubaoLabel);
        ...
        let glmTab: HTMLDivElement = document.createElement('div');
        glmTab.id = 'glmTab';
        glmTab.style.display = 'none';
        rightSide.appendChild(glmTab);
        this.populateGlmTab(glmTab);

        let doubaoTab: HTMLDivElement = document.createElement('div');
        doubaoTab.id = 'doubaoTab';
        doubaoTab.style.display = 'none';
        ...
        glmRadio.addEventListener('change', () => {
            console.log('show GLM tab');
            this.toggleTab('glmTab');
        });
        doubaoRadio.addEventListener('change', () => {
    ...
    populateGlmTab(container: HTMLDivElement): void {
        container.style.paddingTop = '10px';

        let glmDiv: HTMLDivElement = document.createElement('div');
        glmDiv.classList.add('middle');
        glmDiv.classList.add('row');
        glmDiv.style.paddingLeft = '4px';
        glmDiv.innerHTML = '<input type="checkbox" id="enableGlm"><label for="enableGlm" style="padding-top:4px;">Enable GLM Translation</label>';
        ...
        td.classList.add('middle');
        td.classList.add('noWrap');
        td.innerHTML = '<label for="glmKey">API Key</label>';
        tr.appendChild(td);

        td = document.createElement('td');
        td.classList.add('middle');
        td.classList.add('fill_width');
        td.innerHTML = '<input type="text" id="glmKey" class="table_input"/>';
        tr.appendChild(td);
        ...
        td.classList.add('middle');
        td.classList.add('noWrap');
        td.innerHTML = '<label for="glmEndpoint">Endpoint</label>';
        tr.appendChild(td);

        td = document.createElement('td');
        td.classList.add('middle');
        td.classList.add('fill_width');
        td.appendChild(this.glmEndpoint);
        tr.appendChild(td);

        let options: string = '<option value="general">General API</option><option value="coding">Coding Plan API</option>';
        this.glmEndpoint.innerHTML = options;
        ...
        td.classList.add('middle');
        td.classList.add('noWrap');
        td.innerHTML = '<label for="glmModel">GLM Model</label>';
        tr.appendChild(td);
        ...
        let glmModelInput: HTMLInputElement = document.createElement('input');
        glmModelInput.type = 'text';
        glmModelInput.id = 'glmModel';
        glmModelInput.classList.add('table_input');
        glmModelInput.setAttribute('list', 'glmModelsList');
        td.appendChild(glmModelInput);

        let glmModelLink: HTMLAnchorElement = document.createElement('a');
        glmModelLink.href = '#';
        glmModelLink.innerText = 'View Models';
        glmModelLink.style.marginLeft = '8px';
        glmModelLink.style.fontSize = '12px';
        glmModelLink.addEventListener('click', (event: Event) => {
            event.preventDefault();
            ipcRenderer.send('open-link', 'https://docs.bigmodel.cn/cn/guide/start/model-overview');
        });
        td.appendChild(glmModelLink);
        tr.appendChild(td);

        let glmDatalist: HTMLDataListElement = document.createElement('datalist');
        glmDatalist.id = 'glmModelsList';
        container.appendChild(glmDatalist);
        ...
        this.glmFixTags.id = 'glmFixTags';
        this.glmFixTags.type = 'checkbox';
        this.glmFixTags.addEventListener('change', () => {
            if (this.glmFixTags.checked) {
                this.chatGptFixTags.checked = false;
                this.anthropicFixTags.checked = false;
                this.mistralFixTags.checked = false;
                this.geminiFixTags.checked = false;
        ...
        let fixLabel: HTMLLabelElement = document.createElement('label');
        fixLabel.innerText = 'Use to Fix Tags';
        fixLabel.setAttribute('for', 'glmFixTags');
        fixLabel.style.paddingTop = '4px';
        tagsRow.appendChild(fixLabel);

        this.enableGlm = document.getElementById('enableGlm') as HTMLInputElement;
        this.glmKey = document.getElementById('glmKey') as HTMLInputElement;
        this.glmModel = document.getElementById('glmModel') as HTMLInputElement;
    }
    ...
    populateDoubaoTab(container: HTMLDivElement): void {
        ...
        td.classList.add('middle');
        td.classList.add('fill_width');
        td.innerHTML = '<input type="text" id="doubaoKey" class="table_input"/>';
        tr.appendChild(td);
        ...
        td.classList.add('middle');
        td.classList.add('noWrap');
        td.innerHTML = '<label for="doubaoEndpoint">Endpoint</label>';
        tr.appendChild(td);

        td = document.createElement('td');
        td.classList.add('middle');
        td.classList.add('fill_width');
        td.appendChild(this.doubaoEndpoint);
        tr.appendChild(td);

        this.doubaoEndpoint.innerHTML = '<option value="general">General API</option><option value="coding">Coding Plan API</option>';

        this.doubaoEndpoint.addEventListener('change', () => {
        ...
        td.classList.add('middle');
        td.classList.add('noWrap');
        td.innerHTML = '<label for="doubaoModel">Doubao Model</label>';
        tr.appendChild(td);
        ...
        let doubaoModelInput: HTMLInputElement = document.createElement('input');
        doubaoModelInput.type = 'text';
        doubaoModelInput.id = 'doubaoModel';
        doubaoModelInput.classList.add('table_input');
        doubaoModelInput.setAttribute('list', 'doubaoModelsList');
        td.appendChild(doubaoModelInput);

        let doubaoModelLink: HTMLAnchorElement = document.createElement('a');
        doubaoModelLink.href = '#';
        doubaoModelLink.innerText = 'View Models';
        doubaoModelLink.style.marginLeft = '8px';
        doubaoModelLink.style.fontSize = '12px';
        doubaoModelLink.addEventListener('click', (event: Event) => {
            event.preventDefault();
            ipcRenderer.send('open-link', 'https://www.volcengine.com/docs/82379/1330310');
        });
        td.appendChild(doubaoModelLink);
        tr.appendChild(td);

        let doubaoDatalist: HTMLDataListElement = document.createElement('datalist');
        doubaoDatalist.id = 'doubaoModelsList';
        container.appendChild(doubaoDatalist);
        ...
        this.doubaoFixTags.id = 'doubaoFixTags';
        this.doubaoFixTags.type = 'checkbox';
        this.doubaoFixTags.addEventListener('change', () => {
            if (this.doubaoFixTags.checked) {
                this.chatGptFixTags.checked = false;
                this.anthropicFixTags.checked = false;
                this.mistralFixTags.checked = false;
                this.geminiFixTags.checked = false;
        ...
        let fixLabel: HTMLLabelElement = document.createElement('label');
        fixLabel.innerText = 'Use to Fix Tags';
        fixLabel.setAttribute('for', 'doubaoFixTags');
        fixLabel.style.paddingTop = '4px';
        tagsRow.appendChild(fixLabel);
    ...
    applyModelSuggestions(models: { ChatGPT?: string[], Claude?: string[], Mistral?: string[], Gemini?: string[], Qwen?: string[], GLM?: string[], Doubao?: string[], DoubaoCoding?: string[], DeepSeek?: string[] }): void {
        ...
        this.populateModelList('glmModelsList', models.GLM);
        this.doubaoGeneralModels = models.Doubao || [];
    ...
    populateDoubaoModelList(listKey: string): void {
        let models = listKey === 'DoubaoCoding' ? this.doubaoCodingModels : this.doubaoGeneralModels;
        this.populateModelList('doubaoModelsList', models);
    }
