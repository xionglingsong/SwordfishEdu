...
import { FourHorizontalPanels, ThreeVerticalPanels } from "./divider.js";
...
export class TranslationView {
    ...
    static SVG_SUBMIT: string = '<svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 0 24 24" width="24"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z"/></svg>';
    ...
    collapseAiButton: HTMLAnchorElement;
    expandAiButton: HTMLAnchorElement;

    rightHorizontalPanels: FourHorizontalPanels | undefined;
    ...
    aiHiddenBeforePanelCollapse: boolean = false;
    ...
    aiContainer: HTMLDivElement = document.createElement('div');
    ...
    submitButton: HTMLAnchorElement | undefined;
    saveTmButton: HTMLAnchorElement | undefined;
    feedbackPanel: any | undefined;
    scoringTimer: NodeJS.Timeout | undefined;
    scoringElapsed: number = 0;
    scoringQuoteIndex: number = 0;

    constructor(tab: Tab, projectId: string, sourceLang: string, targetLang: string, rows: number, trainingMode: boolean = false) {
        ...
        this.collapseAiButton = document.createElement('a');
        this.expandAiButton = document.createElement('a');
        ...
        ipcRenderer.on('set-score', (_event: IpcRendererEvent, arg: { content: string }) => {
            this.restoreSubmitButton();
            this.aiAssistant?.appendScoringResult(arg.content);
            this.showSaveToTmButton();
        });
        ipcRenderer.on('set-score-structured', (_event: IpcRendererEvent, arg: any) => {
            this.restoreSubmitButton();
            let result = arg.result;
            let summary = '评分：' + result.totalScore + '/100（' + result.level + '）';
            if (result.version === 2 && result.subScores) {
                summary += '\n\n**分项得分：**';
                for (let key of ['meaningTransfer', 'targetMechanics', 'writingQuality']) {
                    let sub = result.subScores[key];
                    if (sub) {
                        summary += '\n- ' + (sub.label || key) + ': ' + sub.score + '/' + sub.max;
                    }
                }
                if (result.errorPoints) summary += '\n\n总扣分: ' + result.errorPoints;
                if (result.qualityPoints && result.qualityPoints > 0) summary += ' | 质量加分: +' + result.qualityPoints;
            }
            let analysisText = result.overallAnalysis || result.overallComment || '';
            if (analysisText) summary += '\n\n' + analysisText;
            summary += '\n\n详细反馈报告已在新窗口中打开。';
            this.aiAssistant?.appendScoringResult(summary);
            this.showSaveToTmButton();
        });
        ipcRenderer.on('score-error', (_event: IpcRendererEvent, arg: { error: string }) => {
            this.restoreSubmitButton();
            this.aiAssistant?.onResponse('评分失败：' + arg.error);
        });
        ipcRenderer.on('tm-save-complete', () => {
            if (this.saveTmButton) {
                this.saveTmButton.innerHTML = TranslationView.SVG_SAVE_TM
                    + '<span style="color:white;font-size:13px;vertical-align:middle">存入记忆库</span>'
                    + '<span class="tooltiptext bottomTooltip">保存到翻译记忆库</span>';
                this.saveTmButton.style.display = 'none';
            }
            this.aiAssistant?.onResponse('译文已成功保存到记忆库。');
        });
        ipcRenderer.on('tm-save-error', (_event: IpcRendererEvent, arg: { error: string }) => {
            if (this.saveTmButton) {
                this.saveTmButton.innerHTML = TranslationView.SVG_SAVE_TM
                    + '<span style="color:white;font-size:13px;vertical-align:middle">存入记忆库</span>'
                    + '<span class="tooltiptext bottomTooltip">保存到翻译记忆库</span>';
            }
            this.aiAssistant?.onResponse('保存失败：' + arg.error);
        });
    ...
    buildTrainingTopBar(): void {
        let saveButton: HTMLAnchorElement = document.createElement('a');
        saveButton.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 0 24 24" width="24">'
            + '<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm4.59-12.42L10 14.17l-2.59-2.58L6 13l4 4 8-8z"/>'
            + '</svg>'
            + '<span class="tooltiptext bottomTooltip">保存</span>';
        saveButton.className = 'tooltip bottomTooltip';
        saveButton.addEventListener('click', () => {
            this.saveEdit({ confirm: false, next: 'none' });
        });
        ...
        reportButton.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 0 24 24" width="24">'
            + '<path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/>'
            + '</svg>'
            + '<span class="tooltiptext bottomTooltip">查看反馈报告</span>';
        reportButton.className = 'tooltip bottomTooltip';
        reportButton.style.marginLeft = '10px';
        reportButton.addEventListener('click', () => {
            ipcRenderer.send('show-score-report', { projectId: this.projectId });
        });
        this.topBar.appendChild(reportButton);

        this.saveTmButton = document.createElement('a');
        this.saveTmButton.innerHTML = TranslationView.SVG_SAVE_TM
            + '<span class="tooltiptext bottomTooltip">存入记忆库</span>';
        this.saveTmButton.className = 'tooltip bottomTooltip';
        this.saveTmButton.style.marginLeft = '10px';
        this.saveTmButton.style.display = 'none';
        this.saveTmButton.addEventListener('click', () => {
            this.saveToTm();
        });
        this.topBar.appendChild(this.saveTmButton);
        ...
        this.submitButton.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 0 24 24" width="24">'
            + '<path d="M7 2v11h3v9l7-12h-4l4-8z"/>'
            + '</svg>'
            + '<span class="tooltiptext">提交AI评分</span>';
        this.submitButton.className = 'tooltip bottomRightTooltip';
        this.submitButton.addEventListener('click', () => {
            this.submitForScoring();
        });
        this.topBar.appendChild(this.submitButton);
    }
    ...
    submitForScoring(): void {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            let sourceCell = row.getElementsByClassName('source')[0] as HTMLTableCellElement | null;
            let targetCell = row.getElementsByClassName('target')[0] as HTMLTableCellElement | null;
            if (sourceCell && targetCell) {
                let target = targetCell.innerText.trim();
                if (target !== '') {
                    segments.push({
                        source: sourceCell.innerText,
                        target: target
                    });
                }
            }
        ...
        if (segments.length === 0) {
            ipcRenderer.send('show-message', { type: 'warning', message: '没有已翻译的段落可评分' });
            return;
        }
        if (segments.length > 20) {
            ipcRenderer.send('show-message', { type: 'info', message: '每次最多评分前20个已翻译段落。' });
            segments = segments.slice(0, 20);
        }
        if (this.submitButton) {
            this.submitButton.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 0 24 24" width="24" style="opacity:0.4"><path d="M7 2v11h3v9l7-12h-4l4-8z"/></svg>'
                + '<span class="tooltiptext">评分中...</span>';
        }
        this.startScoringTimer();
        ipcRenderer.send('score-translation', {
    ...
    static MONTEREY_QUOTES: string[] = [
        '翻译没有标准答案，只有更好的选择。——叶子南',
        '软文本灵活变通，硬文本准确优先。——蒙特雷翻译教学法',
        '理解先于表达，理解错了，表达再好也是徒劳。——李长栓',
        '译者不是传声筒，而是跨文化的桥梁。——蒙特雷翻译理念',
        '翻译的灵活性不是随意性，而是基于理解的创造性选择。——叶子南',
        '一篇散文由五个优秀译者翻译，译文各有特色，难分对错。——叶子南',
        '翻译是一门艺术，也是一门技术，更是一种人文实践。',
        '词汇的对等不等于意义的对等，语境决定一切。——蒙特雷翻译教学法',
        '好的翻译让读者忘记这是翻译，差的翻译让读者想起原文。',
        '隐喻是语言的灵魂，译者要做的不是翻译隐喻，而是翻译隐喻背后的认知。——叶子南',
        '长句分短，短句合并，结构跟着思维走。——中英互译黄金法则',
        '翻译腔不是中文不好，而是被原文的语法绑架了。——蒙特雷翻译教学法',
        '增词不是随意加，减词不是随意删，一切服务于意义传达。——蔡力坚',
        '人工智能可以提供新译本，但无法取代任何一位译者的独特理解。——叶子南',
    ];
    ...
    startScoringTimer(): void {
        ...
        if (this.submitButton) {
            ...
            if (parent) {
                // Hide the filler so scoringStatus can take the space
                let filler = parent.querySelector('.fill_width') as HTMLElement;
                ...
                timerSpan.style.cssText = 'white-space:nowrap;font-weight:bold;color:#FF9800;margin-left:8px;flex-shrink:0;';
                timerSpan.innerText = '0s';
                statusDiv.appendChild(timerSpan);

                parent.insertBefore(statusDiv, this.submitButton);
            }
        }
    ...
    stopScoringTimer(): void {
        if (this.scoringTimer) {
            clearInterval(this.scoringTimer);
            this.scoringTimer = undefined;
        }
        let statusDiv = document.getElementById('scoringStatus');
        if (statusDiv) {
            statusDiv.remove();
        }
        // Restore the filler
        if (this.submitButton) {
            let filler = this.submitButton.parentElement?.querySelector('.fill_width') as HTMLElement;
    ...
    showSaveToTmButton(): void {
        if (this.saveTmButton) {
            this.saveTmButton.style.display = '';
        }
    }
    ...
    saveToTm(): void {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (sourceCell && targetCell) {
                let target = targetCell.innerText.trim();
                if (target !== '') {
                    segments.push({ source: sourceCell.innerText, target: target });
                }
            }
        }
        if (segments.length === 0) {
            ipcRenderer.send('show-message', { type: 'warning', message: 'No translated segments to save' });
            return;
        }
        if (this.saveTmButton) {
            this.saveTmButton.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" height="16" viewBox="0 0 24 24" width="16"><path fill="white" d="M12 4V1L8 5l4 4V6c3.31 0 6 2.69 6 6 0 1.01-.25 1.97-.7 2.8l1.46 1.46C19.54 15.03 20 13.57 20 12c0-4.42-3.58-8-8-8zm0 14c-3.31 0-6-2.69-6-6 0-1.01.25-1.97.7-2.8L5.24 7.74C4.46 8.97 4 10.43 4 12c0 4.42 3.58 8 8 8v3l4-4-4-4v3z"/></svg>'
                + '<span style="color:white;font-size:12px;margin-left:4px;">保存中...</span>';
        }
        ipcRenderer.send('save-training-to-tm', {
            projectId: this.projectId,
            segments: segments,
            srcLang: this.srcLang,
            tgtLang: this.tgtLang
    ...
    buildRightSide(): void {
        ...
        let horizontalSplit: FourHorizontalPanels = new FourHorizontalPanels(panelsContainer);
        ...
        horizontalSplit.fourthPanel().dataset.collapsed = 'false';
        ...
        this.createAiAssistantArea(horizontalSplit.fourthPanel());

        if (this.trainingMode) {
            horizontalSplit.collapseTop();
            this.matchesContainer.classList.add('hidden');
            horizontalSplit.collapseCenter();
            this.mtContainer.classList.add('hidden');
            horizontalSplit.collapseBottom();
        ...
        this.toggleButtonVisibility(this.expandAiButton, false);
    }
    ...
    createMemoryArea(topPanel: HTMLDivElement): void {
        ...
        this.collapseRightButton.addEventListener('click', () => {
            ...
            this.aiHiddenBeforePanelCollapse = this.aiContainer.classList.contains('hidden');
            this.matchesContainer.classList.add('hidden');
            this.mtContainer.classList.add('hidden');
            this.termsContainer.classList.add('hidden');
            ...
            const observer: MutationObserver = new MutationObserver((mutationsList: MutationRecord[]) => {
                for (const mutation of mutationsList) {
                    ...
                    if (panelWidth > TranslationView.MIN_PANEL_WIDTH) {
                        ...
                        if (this.aiHiddenBeforePanelCollapse) {
                            this.aiContainer.classList.add('hidden');
                        } else {
    ...
    createAiAssistantArea(fourthPanel: HTMLDivElement): void {
        let panelsContainer: HTMLDivElement = document.createElement('div');
        panelsContainer.classList.add('fourthPaddedPanel');
        fourthPanel.appendChild(panelsContainer);

        let aiTitle: HTMLDivElement = document.createElement('div');
        aiTitle.classList.add('titlepanel');
        aiTitle.style.overflow = 'visible';
        aiTitle.style.display = 'flex';
        panelsContainer.appendChild(aiTitle);

        let blankFiller: HTMLSpanElement = document.createElement('span');
        blankFiller.innerHTML = TranslationView.SVG_BLANK;
        aiTitle.appendChild(blankFiller);

        let Title: HTMLSpanElement = document.createElement('span');
        Title.classList.add('fill_width');
        Title.innerText = 'AI Tutor';
        aiTitle.appendChild(Title);

        this.expandAiButton.className = 'tooltip topRightTooltip';
        this.expandAiButton.innerHTML = TranslationView.SVG_VERTICAL_EXPAND + '<span class="tooltiptext topRightTooltip">Expand AI Assistant</span>';
        this.expandAiButton.style.marginLeft = '4px';
        this.toggleButtonVisibility(this.expandAiButton, false);
        this.expandAiButton.addEventListener('click', () => {
            if (!this.aiContainer.classList.contains('hidden')) {
                return;
            }
            this.aiContainer.classList.remove('hidden');
            this.rightHorizontalPanels?.expandFourth();
            this.updateRightPanelsLayout();
            this.aiHiddenBeforePanelCollapse = false;
        });
        aiTitle.appendChild(this.expandAiButton);

        this.collapseAiButton.className = 'tooltip bottomRightTooltip';
        this.collapseAiButton.innerHTML = TranslationView.SVG_VERTICAL_COLLAPSE + '<span class="tooltiptext bottomRightTooltip">Collapse AI Assistant</span>';
        this.collapseAiButton.style.marginLeft = '4px';
        this.collapseAiButton.addEventListener('click', () => {
            if (this.aiContainer.classList.contains('hidden')) {
                return;
            }
            this.aiContainer.classList.add('hidden');
            this.rightHorizontalPanels?.collapseFourth();
            this.updateRightPanelsLayout();
            this.aiHiddenBeforePanelCollapse = true;
        });
        aiTitle.appendChild(this.collapseAiButton);

        this.aiContainer.classList.add('fill_width');
        panelsContainer.appendChild(this.aiContainer);
        this.aiAssistant = new AiAssistant(this.aiContainer, () => {
            let sourceCell = this.currentRow?.getElementsByClassName('source')[0] as HTMLTableCellElement | null;
            return {
                sourceText: sourceCell?.innerText || '',
                targetText: this.currentCell?.innerText || '',
                srcLang: this.srcLang,
                tgtLang: this.tgtLang
            };
        }, (text: string) => {
        ...
        let observer: MutationObserver = new MutationObserver((mutationsList) => {
            for (let mutation of mutationsList) {
                if (mutation.type === 'attributes') {
                    const availableHeight: number = panelsContainer.clientHeight - aiTitle.clientHeight;
                    this.aiContainer.style.height = availableHeight + 'px';
                    if (this.aiContainer.classList.contains('hidden') && availableHeight > TranslationView.MIN_SUBPANEL_HEIGHT) {
                        this.aiContainer.classList.remove('hidden');
                        this.rightHorizontalPanels?.expandFourth();
                        this.aiHiddenBeforePanelCollapse = false;
                        this.updateRightPanelsLayout();
        ...
        observer.observe(fourthPanel, config);
    }
    ...
    updateRightPanelsLayout(skipAutoRestore: boolean = false): void {
        ...
        const aiHidden: boolean = this.aiContainer.classList.contains('hidden');

        if (!skipAutoRestore && matchesHidden && mtHidden && termsHidden && aiHidden) {
            this.matchesContainer.classList.remove('hidden');
            this.mtContainer.classList.remove('hidden');
            this.termsContainer.classList.remove('hidden');
            this.aiContainer.classList.remove('hidden');
            ...
            if (this.rightHorizontalPanels) {
                ...
                this.rightHorizontalPanels.fourthPanel().dataset.collapsed = 'false';
                ...
                this.rightHorizontalPanels.expandedFourth = 0;
                this.rightHorizontalPanels.setWeights([25, 25, 25, 25]);
            }
            this.updateRightPanelsLayout(true);
            return;
        }
        ...
        toggleButtons(aiHidden, this.collapseAiButton, this.expandAiButton);
        ...
        if (this.rightHorizontalPanels) {
            ...
            this.rightHorizontalPanels.fourthPanel().dataset.collapsed = aiHidden ? 'true' : 'false';
        }
    }
    ...
    selectRow(row: HTMLTableRowElement): void {
        ...
        let sourcePreview = (this.currentRow.getElementsByClassName('source')[0] as HTMLTableCellElement)?.innerText || '';
        this.aiAssistant?.setCurrentSegment(sourcePreview);
